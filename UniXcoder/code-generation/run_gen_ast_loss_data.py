...
from utils import filter_encoder_state_dict,get_elapse_time,filter_Roberta_encoder_state_dict,calculate_ast_loss,read_concode_examples
...
def main():
    ...
    logger.info(f"Successfully imported parameters from fine-tuned network {params_file}")
    ...
    if args.do_train:
        ...
        # 定义文件保存路径
        output_path = '/AIsoftwaremfq2023/code/cl_code/UniXcoder/dataset/ast_loss_data/train.pt'
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for idx, batch in enumerate(bar):
                ...
                outputs = model.module.get_hidden_state(source_ids=source_ids, target_ids=target_ids)
                ast_loss = calculate_ast_loss(outputs, target_ids, tokenizer)
                # print(outputs.size())  #[24, 380, 51416]
                pooled_logits = torch.mean(outputs, dim=1)
                # print(pooled_logits.size()) #[24, 51416]

                # 保存 pooled_logits 和 ast_loss
                pooled_logits_list.append(pooled_logits.detach())
                ast_loss_list.append(ast_loss.detach())

                # 每 batch_size_to_save 个批次保存一次数据
                output_dir = '/AIsoftwaremfq2023/code/cl_code/UniXcoder/code-generation/dataset/ast_loss_data/concode'
                if (idx + 1) % batch_size_to_save == 0:
                    save_path = os.path.join(output_dir, f"batch_{idx + 1}.pt")
                    torch.save({
                        'pooled_logits': torch.cat(pooled_logits_list),
                        'ast_loss': torch.cat(ast_loss_list)
                    }, save_path)
                    save_count += 1
                    # 清空列表，释放内存
                    pooled_logits_list = []
            ...
            # 保存剩余数据
            if pooled_logits_list and ast_loss_list:
                output_dir = '/AIsoftwaremfq2023/code/cl_code/UniXcoder/code-generation/dataset/ast_loss_data/concode'
        ...
        #     'pooled_logits': torch.cat(pooled_logits_list),
        #     'ast_loss': torch.cat(ast_loss_list)
        # }, '/AIsoftwaremfq2023/code/cl_code/UniXcoder/dataset/ast_loss_data/train.pt')
