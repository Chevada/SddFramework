...
def convert_siamese_examples_to_features(item):
    example, example_index, tokenizer, args, stage = item

    if args.model_type in ['t5', 'codet5'] and args.add_task_prefix:
        if args.sub_task != 'none':
            source_str = "{} {}: {}".format(args.task, args.sub_task, example.comment)
        else:
            source_str = "{}: {}".format(args.task, example.comment)
    else:
        source_str = example.comment

    ast_str = example.ast

    source_str = source_str.replace('</s>', '<unk>')
    ast_str = ast_str.replace('</s>', '<unk>')
    source_ids = tokenizer.encode(source_str, max_length=args.max_source_length, padding='max_length', truncation=True)
    ast_ids = tokenizer.encode(ast_str, max_length=args.max_target_length, padding='max_length', truncation=True)
    assert source_ids.count(tokenizer.eos_token_id) == 1
    assert ast_ids.count(tokenizer.eos_token_id) == 1

    target = int(example.target)

    return SiameseInputFeatures(
        example_index,
        source_ids,
        ast_ids,
        target,
...
# 这是自己定义的读取文件方法
def read_gen_class_examples(filename, data_num):
    """Read examples from filename."""
    examples = []
...
def read_siamese_examples(filename,data_num):
    """Read examples from filename."""
    examples = []
