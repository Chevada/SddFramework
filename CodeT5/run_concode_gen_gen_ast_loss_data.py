...
此文件用于使用初步微调完成的codet5模型，生成用于语法判别器训练的数据
...
import pickle
from utils import get_filenames, get_elapse_time, load_and_cache_gen_data,filter_encoder_state_dict,filter_decoder_state_dict,weighted_edit_distance_loss
...
def calculate_ast_loss(outputs,target_ids,tokenizer):
    ...
    # 批量生成 AST
    generated_asts = [generate_single_enhanced_ast(code, 'java').split(' ') for code in generated_codes]
    target_asts = [generate_single_enhanced_ast(code, 'java').split(' ') for code in target_codes]
...
def main():
    ...
    params_path = 'saved_models/concode/codet5_base_all_lr3_bs24_src320_trg150_pat3_e30_20241219_121743'
    ...
    if args.do_train:
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(bar):
                ...
                if args.model_type == 'roberta':
                    ...
                    output_dir = '/AIsoftwaremfq2023/code/cl_code/CodeT5/data/ast_loss_data/java-train'
                    if (step + 1) % batch_size_to_save == 0:
                        save_path = os.path.join(output_dir, f"batch_{step + 1}.pt")
                        torch.save({
        ...
        torch.save({
            'pooled_logits': torch.cat(pooled_logits_list),
            'ast_loss': torch.cat(ast_loss_list)
        }, '/AIsoftwaremfq2023/code/cl_code/CodeT5/data/ast_loss_data/train.pt')
