...
此文件用于使用SDD框架，在concode数据集上，基于codet5-base模型进行训练
"""
...
def eval_result(args, eval_data, eval_examples, model, tokenizer, split_tag, criteria):
    args.res_dir = 'saved_models/concode/codet5_base_all_lr10_bs16_src320_trg150_pat3_e30/prediction'
    output_fn = os.path.join(args.res_dir, "test_{}.output".format(criteria))
    gold_fn = os.path.join(args.res_dir, "test_{}.gold".format(criteria))
    src_fn = os.path.join(args.res_dir, "test_{}.src".format(criteria))
    # 初始化 dev_accs 列表
...
def main():
    ...
    params_path = 'saved_models/concode/codet5_base_all_lr10_bs24_src320_trg150_pat3_e30_20241213_082112'
    ...
    if args.do_test:
        # args.output_dir = '/AIsoftwaremfq2023/code/cl_code/CodeT5/sh/saved_models/concode/codet5_base_all_lr10_bs16_src320_trg150_pat3_e30'
        logger.info("  " + "***** Testing *****")
        logger.info("  Batch size = %d", args.eval_batch_size)
