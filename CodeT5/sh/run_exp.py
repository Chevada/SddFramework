...
def get_siamese_cmd(task, sub_task, model_tag, gpu, data_num, bs, lr, source_length, target_length, patience, epoch, warmup,
            model_dir, summary_dir, res_fn,loss_fn, max_steps=None, save_steps=None, log_steps=None):
    if max_steps is None:
        cmd_str = 'bash exp_with_args.sh %s %s %s %d %d %d %d %d %d %d %d %d %s %s %s %s' % \
                  (task, sub_task, model_tag, gpu, data_num, bs, lr, source_length, target_length, patience, epoch,
                   warmup, model_dir, summary_dir, res_fn,loss_fn)
    else:
        cmd_str = 'bash exp_with_args.sh %s %s %s %d %d %d %d %d %d %d %d %d %s %s %s %d %d %d' % \
                  (task, sub_task, model_tag, gpu, data_num, bs, lr, source_length, target_length, patience, epoch,
                   warmup, model_dir, summary_dir, res_fn, max_steps, save_steps, log_steps)
...
def get_args_by_task_model(task, sub_task, model_tag):
    if task == 'translate':
        ...
        epoch = 1
        patience = 2
    elif task == 'gen_class' or task == 'sia_gen':
        # 这是去掉特殊字符的生成
        # Read 15282 examples, avg src len: 15, avg trg len: 45, max src len: 234, max trg len: 225
        # [TOKENIZE] avg src len: 31, avg trg len: 149, max src len: 300, max trg len: 495
        src_len = 64
        trg_len = 380
        epoch = 40
        patience = 3
    elif task == 'siamese':
        # Read 42012 examples, avg_com_len: 21, avg_ast_len: 46, max src len: 162, max trg len: 289
        # [TOKENIZE] avg_com_len: 40, avg_ast_len: 147, max_com_len: 439, max_ast_len: 927
        # pyclass训练
        src_len = 64
        trg_len = 150
        # concode训练
    ...
    if 'codet5_small' in model_tag:
        ...
        if task == 'translate':
            ...
            bs = 40
            lr = 10
            loss_fn = 'euc' # 可选值为euc或者cos，分别表示使用欧氏距离或者余弦相似度进行训练
            return bs, lr, src_len, trg_len, patience, epoch,loss_fn
        elif task == 'summarize':
            bs = 48
        elif task == 'clone':
...
def run_siamese_exp(args):
    bs, lr, src_len, trg_len, patience, epoch,loss_fn= get_args_by_task_model(args.task, args.sub_task, args.model_tag)
    print('============================Start Running==========================')
    cmd_str = get_siamese_cmd(task=args.task, sub_task=args.sub_task, model_tag=args.model_tag, gpu=args.gpu,
                      data_num=args.data_num, bs=bs, lr=lr, source_length=src_len, target_length=trg_len,
                      patience=patience, epoch=epoch, warmup=1000,
                      model_dir=args.model_dir, summary_dir=args.summary_dir,
                      res_fn='{}/{}_{}.txt'.format(args.res_dir, args.task, args.model_tag),
...
def get_sub_tasks(task):
    ...
    elif task in ['concode', 'defect', 'clone', 'multi_task','gen_class','siamese','sia_gen']:
        sub_tasks = ['none']
    return sub_tasks
...
if __name__ == '__main__':
    ...
    parser.add_argument("--task", type=str, default='summarize', choices=['summarize', 'concode', 'translate',
                                                                          'refine', 'defect', 'clone', 'multi_task','gen_class','siamese','sia_gen'])
    ...
    # 在命令行显示执行run_exp.py文件，实际在该文件中只是重新处理了需要的命令行参数，起到简化调用的目的
    # 最终该文件使用os.system（bash exp_with_args.sh 处理好的命令行参数），实际上执行的是exp_with_args.sh文件

    if not os.path.exists(args.res_dir):
        os.makedirs(args.res_dir)

    assert args.sub_task in get_sub_tasks(args.task)
    if args.task == 'siamese' or args.task=='sia_gen':
        run_siamese_exp(args)
