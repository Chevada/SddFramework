...
def main():
    ...
    if args.do_train:
        ...
        ast_optimizer = AdamW(astLossModel.parameters(), lr=0.01)
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            # 每个 epoch 随机选择五分之一的 batch
            total_batches = len(train_dataloader)
            num_selected_batches = total_batches // 5  # 修改为五分之一
            selected_batches = random.sample(range(total_batches), num_selected_batches)
            ...
            for step, batch in enumerate(bar):
                ...
                if args.model_type == 'roberta':
                    ...
                    outputs = model(input_ids=source_ids, attention_mask=source_mask,
                                    labels=target_ids, decoder_attention_mask=target_mask)
                    loss = outputs.loss
                    if step in selected_batches:
                        if cur_epoch<10 or fcn_loss_total / (step + 1)>0.01:
                            ast_loss = calculate_ast_loss(outputs, target_ids, tokenizer)
                            pooled_logits = torch.mean(outputs.logits, dim=1)
                            # predicted_ast_loss = astLossModel(outputs.logits[:, -1, :])
                            predicted_ast_loss = astLossModel(pooled_logits)
                            predicted_ast_loss = predicted_ast_loss.squeeze()  # 去掉维度为1的维度
                            ast_loss = ast_loss.to(predicted_ast_loss.device)
                            ...
                            fcn_loss = mse_loss_fn(scaled_predicted_ast_loss, scaled_ast_loss)

                            # 记录 fcn_loss
                            fcn_loss_total += fcn_loss.item()  # 累计 fcn_loss

                            if args.n_gpu > 1:
                                fcn_loss = fcn_loss.mean()  # mean() to average on multi-gpu.
                            if args.gradient_accumulation_steps > 1:
                                fcn_loss = fcn_loss / args.gradient_accumulation_steps
                            # 此处仅仅对astLossModel进行参数更新
                            ast_optimizer.zero_grad()  # 清除 astLossModel 的梯度
                            # fcn_loss.backward(retain_graph=True)  # 反向传播
                            fcn_loss.backward()  # 反向传播
                            ast_optimizer.step()  # 更新 astLossModel 的参数
                            optimizer.zero_grad()  # 清除 CodeT5 的梯度
                        else:
                            ...
                            predicted_ast_loss = astLossModel(outputs.logits)
                    ...
                    bar.set_description("[{}] Train loss {},FCN loss {}, Predicted AST loss {}".format(
                        cur_epoch, round(train_loss, 3),
                        round(fcn_loss_total / (step + 1), 3) if step > 0 else 0,
            ...
            # 每四个轮次修改code_loss和ast_loss的比重
            if (cur_epoch + 1) % 4 == 0 and code_weight > 0.1:
                code_weight -= 0.1
                ast_weight += 0.1
    ...
    if args.do_test:
        ...
        # args.output_dir = 'saved_models/gen_class/codet5_base_all_lr3_bs24_src64_trg350_pat3_e40_20241016_111224'
        for criteria in ['best-ppl']:
