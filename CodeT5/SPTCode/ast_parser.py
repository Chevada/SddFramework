...
import re
import SPTCode.enums as enums


# LANGUAGE = {enums.LANG_PYTHON: Language('/AIsoftwaremfq2023/code/cl_code/CodeT5/SPTCode/build/my-languages.so', 'python')}
LANGUAGE = {enums.LANG_JAVA: Language('/AIsoftwaremfq2023/code/cl_code/CodeT5/SPTCode/build/my-languages.so', 'java')}
...
#             enums.LANG_PYTHON: Language('/AIsoftwaremfq2023/code/cl_code/CodeT5/SPTCode/build/my-languages.so', 'python'),
...
STATEMENT_ENDING_STRINGS = {
    ...
    # 在此添加节点
    enums.LANG_PYTHON: ['statement', 'assignment','definition','clause'],
...
def filter_source_code(source):
    # 移除所有非 UTF-8 字符
    return re.sub(r'[^\u0000-\uD7FF\uE000-\uFFFF]', '', source)

# 方法名有enhanced为改进的方法
def generate_single_enhanced_ast(source, lang):
    """
    ...
    ast = generate_enhanced_statement_xsbt(node=root, lang=lang)
    return ast

def generate_enhanced_statement_xsbt(node, lang):
    """
    ...
    #     node = captures[0][0]
    tokens = __enhanced_statement_xsbt(node=node, lang=lang)
    return ' '.join(tokens)
...
def __enhanced_statement_xsbt(node, lang):
    """
    ...
    if len(node.children) == 0:
        # 忽略 `expression_statement` 节点
        if is_statement_node(node, lang) and get_node_type(node, lang) != 'expression_statement':
            xsbt.append(to_pascal_case(get_node_type(node, lang)))
    else:
        # 忽略 `expression_statement` 节点
        if is_statement_node(node, lang) and get_node_type(node, lang) != 'expression_statement':
            xsbt.append(f'Begin{to_pascal_case(get_node_type(node, lang))}')

        # 记录当前序列长度
        len_before = len(xsbt)
        for child in node.children:
            # 递归对子节点进行处理
            xsbt += __enhanced_statement_xsbt(node=child, lang=lang)

        # 如果当前节点不为 `expression_statement` 且没有子节点，更新类型
        if len_before == len(xsbt) and len_before != 0 and get_node_type(node, lang) != 'expression_statement':
            xsbt[-1] = to_pascal_case(get_node_type(node, lang))
        elif is_statement_node(node, lang) and get_node_type(node, lang) != 'expression_statement':
            # 如果是声明节点且类型不为 `expression_statement`，在末尾添加格式为 `__{节点类型}`
            xsbt.append(f'End{to_pascal_case(get_node_type(node, lang))}')
...
def parse_ast(source, lang):
    ...
    tree = parser.parse(filter_source_code(source).encode('utf-8').decode('unicode_escape', errors='ignore').encode())
    root = tree.root_node
    # tree = parser.parse(str.encode(source))
    # 注释这段，在解析生成的java ast时，会报错：root = captures[0][0]  IndexError: list index out of range
...
def is_statement_node(node, lang):
    ...
    """
    # python的statement结束字符串：['statement', 'assignment'],
...
def __statement_xsbt(node, lang):
    ...
    if len(node.children) == 0:
        # 递归到最后一层，判断是不是声明节点，即判断是不是声明语句或者赋值语句['statement', 'assignment'],
        # 判断
        #     endings = ['statement', 'assignment']
        #     end = node.type.split('_')[-1]是否是'statement'或者'assignment'
        if is_statement_node(node, lang):
            xsbt.append(get_node_type(node, lang))
    else:
        if is_statement_node(node, lang):
            # 如果是声明节点，向序列中添加一个带有特殊格式的字符串，格式为{节点类型}__。
        ...
        if len_before == len(xsbt) and len_before != 0:
            # 如果处理完所有子节点后，序列的长度没有变化（即没有子节点是声明节点或者对序列有贡献的节点）且序列不为空，将序列的最后一个元素更新为当前节点的类型。
            # 基本逻辑就是，只会向ast序列中添加statement或者assignment节点，其他节点都不会添加
            xsbt[-1] = get_node_type(node, lang)
        elif is_statement_node(node, lang):
            # 如果当前节点是声明节点，并且在处理子节点后序列的长度发生了变化，就在序列末尾添加一个格式为__{节点类型}的字符串。
